...
class ProstNFound(nn.Module):
    ...
    def __init__(
        ...
        self.native_image_size = native_image_size
    ...
    def forward(
        ...
        **prompts,
        ...
        if (H != self.native_image_size or W != self.native_image_size):
            if not self._WARNED_RESIZE: 
                warn(f"Detected image of resolution {H, W} which is different from native image encoder resolution {self.native_image_size, self.native_image_size}")
                self._WARNED_RESIZE = True
            if self.auto_resize_image_to_native:
                image_resized_for_features = torch.nn.functional.interpolate(
                    image, size=(self.native_image_size, self.native_image_size)
                )
            else: 
